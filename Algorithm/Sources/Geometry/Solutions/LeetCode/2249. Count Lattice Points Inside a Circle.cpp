...
struct Circle
{
	...
	void addPositions(vector<Vec>& v) const
	{
...
public:
	int countLatticePoints(vector<vector<int>>& circles) 
	{
		...
		for(int i = 0; i < circles.size(); ++i)
		{
			...
			Circle circle(Vec(x, y), radius);
			circle.addPositions(positions);
		}

		sort(begin(positions), end(positions));
		positions.erase(unique(begin(positions), end(positions)), end(positions));
