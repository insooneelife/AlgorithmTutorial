...
// i, j 길에 방향제한이 있다면,
// d의 방향이 Down인 경우, 이전 상태에서도 Down이어야 한다.
// F(i, j, d) = F(i - 1, j, Down)
// d의 방향이 Right인 경우, 이전 상태에서도 Right이어야 한다.
// F(i, j, d) = F(i, j - 1, Right)

// i, j 길이 열려있다면,
// F(i, j, d) = F(i - 1, j, Down) + F(i, j - 1, Right)
// (위에서 아래로 내려오는 경우의 수) + (왼쪽에서 오른쪽으로 오는 경우의 수)
